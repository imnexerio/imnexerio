...
def get_recent_repos(g, user, days=30):
                ...
                if repo.description:
                    ...
                    if any(word in desc_lower for word in ["learning", "practicing", "studying"]):
                        ...
                        for tech in ["kotlin", "python", "javascript", "react", "flutter", "android",
                                     "tensorflow", "pytorch", "ml", "ai", "vue", "backend", "frontend", "fullstack"]:
                            if tech in desc_lower:
                                learning_topics.add(tech)

                # Add main language as potential learning topic if it's recent
...
def get_language_stats(g, user):
    """Get language statistics for the user's repositories"""
    user_obj = g.get_user(user)
    repos = user_obj.get_repos()
    ...
    for repo in repos:
        try:
            ...
            print(f"Error getting language for repo {repo.name}: {str(e)}")
            continue
...
def update_readme(user):
    ...
    try:
        ...
        recent_repos, learning_topics = get_recent_repos(g, user)
        ...
        # Learning line based on detected topics or default to Vue (from your original README)
        if learning_topics:
            topics_str = ", ".join([topic for topic in learning_topics][:3])
            current_work_lines.append(f"- 🌱 I'm currently learning {topics_str}")
        else:
            current_work_lines.append("- 🌱 I'm currently learning vue")
